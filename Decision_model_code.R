...
#read in the inputs####
#and reformat it from csv2 to csv
#(required due to the use of a German computer)

#general inputs 
Frost_Input<-read.csv("Frost_protection_input_apple_supplementary.csv")

#frost scenarios Rhineland and Lake_Constance 
Scenario_input<-read.csv("frost_scenario_supplementary.csv")


#prepare Input table for both scenarios
#Rhineland
Scenario_input_Rhineland<-subset(Scenario_input, Scenario_input$comment=="Scenario_Rhineland")
Frost_Input_Rhineland<-rbind(Frost_Input, Scenario_input_Rhineland)
#Lake Constance
Scenario_input_Lake_Constance<-subset(Scenario_input, Scenario_input$comment=="Scenario_Lake_Constance")
Frost_Input_Lake_Constance<-rbind(Frost_Input, Scenario_input_Lake_Constance)

#make_variables(as.estimate(Frost_Input_Rhineland))
#make_variables(as.estimate(Frost_Input_Lake_Constance))

#information Input
Information_input<-read.csv("Information_input_apple_supplementary.csv")
plot_area<-subset(Information_input$Value,
                  Information_input$Information=="plot_area")
terrain_correction_needed<-subset(Information_input$Value,
                                  Information_input$Information=="terrain_correction_needed")
existing_irrigation_pond<-subset(Information_input$Value,
                                 Information_input$Information=="existing_irrigation_pond")
groundwater_well<-subset(Information_input$Value,
                         Information_input$Information=="groundwater_well")
#decision function####
#for the decision between apple production without frost protection, candles,
#tractor-mounted gas heaters, portable gas heaters, mobile/stationary wind machines
#overhead irrigation,below-canopy irrigation or pellet heaters. 
frost_protection_apple<-function(){
  ...
  #Frostbuster (tailed or 3 point mounted; tractor-mounted)####
  ...
  #Frostguard (stationary; portable gas heater) (the bigger version with 4 gas bottles)####
...
#Monte Carlo simulation with 100000 runs
frost_protection_mc_simulation_scenario_Rhineland_100000 <- mcSimulation(estimate = as.estimate(Frost_Input_Rhineland),
                                                                   model_function = frost_protection_apple,
                                                                   numberOfModelRuns = 100000,
                                                                   functionSyntax = "plainNames")
#save
write.csv(frost_protection_mc_simulation_scenario_Rhineland_100000, "results/MC_investments_apple_scenario_Rhineland_100000.csv")
saveRDS(frost_protection_mc_simulation_scenario_Rhineland_100000, "results/Monte_Carlo_apple_scenario_Rhineland_100000.RDS")

#Scenario Lake_Constance####

#Monte Carlo simulation  100000 runs
frost_protection_mc_simulation_scenario_Lake_Constance_100000 <- mcSimulation(estimate = as.estimate(Frost_Input_Lake_Constance),
                                                            model_function = frost_protection_apple,
                                                            numberOfModelRuns = 100000,
                                                            functionSyntax = "plainNames")
#save
write.csv(frost_protection_mc_simulation_scenario_Lake_Constance_100000, "results/MC_investments_apple_scenario_Lake_Constance_100000.csv")
saveRDS(frost_protection_mc_simulation_scenario_Lake_Constance_100000, "results/Monte_Carlo_apple_scenario_Lake_Constance_100000.RDS")
